...
"[project]/src/app/api/loans/available/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    ...
    try {
        ...
        if (!profileId || !(0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$fm$2f$security$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__["isValidUserId"])(profileId)) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        ...
        try {
          ...
          personality
        `).eq('is_on_loan_market', true).neq('profile_id', profileId);
            players = result.data || [];
        ...
        if (playersError) {
            ...
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: 'Kiralık oyuncular yüklenirken hata oluştu'
            }, {
                status: 500
            });
        }
        // ── Zaten kirada olan oyuncuları filtrele ──
        const availablePlayers = (players || []).filter((p)=>p.loan_status !== 'active');
        // ── Her oyuncu için sahibin takım adını getir ──
        const ownerIds = [
            ...new Set(availablePlayers.map((p)=>p.loan_owner_profile_id || p.profile_id).filter(Boolean))
        ];
        let ownerProfiles = {};
        if (ownerIds.length > 0) {
            const { data: profiles } = await supabase.from('profiles').select('id, team_name').in('id', ownerIds);
            if (profiles) {
        ...
        const enrichedPlayers = availablePlayers.map((p)=>{
            const ownerId = p.loan_owner_profile_id || p.profile_id;
...
//# sourceMappingURL=%5Broot-of-the-server%5D__78c78886._.js.map