...
"[project]/src/app/api/league/standings/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    ...
    if (profileId && !leagueId && supabase) {
        const userLeagueId = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$fm$2f$leagueHelpers$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getUserLeagueId"])(supabase, profileId);
        if (userLeagueId) {
    ...
    try {
        ...
        if (!isUuid && leagueId && leagueId !== '') {
            ...
            const { data: byId } = await supabase.from('leagues').select('id, name, tier').eq('id', leagueId).maybeSingle();
            if (byId) {
                finalLeagueId = byId.id;
            } else {
                // Try as integer ID
                const { data: byIntId } = await supabase.from('leagues').select('id, name, tier').eq('id', parseInt(leagueId)).maybeSingle();
                if (byIntId) {
                    finalLeagueId = byIntId.id;
                } else {
                    // Otherwise treat as tier number
                    const leagueTier = parseInt(leagueId);
                    const { data: tierLeagues, error: tierError } = await supabase.from('leagues').select('id').eq('tier', leagueTier).order('name', {
                        ascending: true
                    });
            ...
            const { data: firstLeague } = await supabase.from('leagues').select('id').eq('tier', 1).order('id', {
                ascending: true
            }).limit(1).single();
        ...
        return (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$api$2d$error$2d$handler$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createErrorResponse"])(error, {
            route: '/api/league/standings',
            method: 'GET'
        });
    }
}
async function getStandingsForLeague(supabase, leagueId) {
    ...
    const { data: leagueRow } = await supabase.from('leagues').select('id').eq('id', leagueId).maybeSingle();
    if (!leagueRow) {
        const { data: leagueRow2 } = await supabase.from('leagues').select('id').eq('id', parseInt(leagueId)).maybeSingle();
        if (leagueRow2) {
            resolvedLeagueId = leagueRow2.id;
        }
    }
    console.log(`[standings] Resolved leagueId: ${resolvedLeagueId}`);
    const { data: seasonData, error: seasonError } = await supabase.from('seasons').select('id').eq('league_id', resolvedLeagueId).order('created_at', {
        ascending: false
    }).limit(1).single();
    ...
    const { data: standings, error: standingsError } = await supabase.from('league_standings').select(`
      id,
      team_id,
      played,
      won,
    ...
    `).eq('season_id', seasonData.id).order('points', {
        ascending: false
    }).order('gd', {
        ascending: false
    }).order('gf', {
    ...
    const { data: leagueInfo } = await supabase.from('leagues').select('name, tier').eq('id', resolvedLeagueId).single();
    ...
    const { data: allLeagueTeams } = await supabase.from('league_teams').select('id, name, profile_id, is_npc').eq('league_id', resolvedLeagueId);
    const teamNameMap = {};
    const teamProfileIdMap = {};
    if (allLeagueTeams) {
    ...
    const profileIds = (allLeagueTeams || []).filter((t)=>t.profile_id).map((t)=>t.profile_id);
    if (profileIds.length > 0) {
        try {
            const { data: profileRows } = await supabase.from('profiles').select('id, team_name').in('id', profileIds);
            if (profileRows) {
    ...
    const formattedStandings = (standings || []).map((s)=>{
        ...
        if (teamName === 'Bilinmiyor') {
            ...
            const stableIndex = (allLeagueTeams || []).findIndex((t)=>t.id === s.team_id);
...
async function getAllLeagues(supabase) {
    const { data } = await supabase.from('leagues').select('*').order('tier', {
        ascending: true
    });
    return data || [];
}
...
//# sourceMappingURL=%5Broot-of-the-server%5D__5cddada8._.js.map