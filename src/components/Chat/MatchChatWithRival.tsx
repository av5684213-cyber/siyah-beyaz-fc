                  ...
                  <motion.div key="dm-chat" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="flex-1 flex flex-col overflow-hidden">
                      ...
                      {dmMessages.map(msg => {
                        ...
                        return (
                          <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'} group`}>
                            <div className={`max-w-[85%] relative ${isMine ? 'bg-red-600/20 border border-red-500/20 rounded-2xl rounded-br-sm' : 'bg-white/5 border border-white/5 rounded-2xl rounded-bl-sm'}`}>
                    ...
                    <div className="p-2.5 bg-zinc-900/50 border-t border-white/5 shrink-0">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button onClick={handleDmSend} disabled={!dmInput.trim() || isSending} className="shrink-0 w-9 h-9 bg-red-600 hover:bg-red-500 disabled:bg-white/5 disabled:text-white/10 rounded-xl flex items-center justify-center transition-all active:scale-95">
                          <Send size={14} className="text-white" />
                        </button>
                      </div>
                    </div>
