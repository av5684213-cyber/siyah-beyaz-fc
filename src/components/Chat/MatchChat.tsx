...
import { Send, MessageSquare, Zap, X, ChevronDown } from 'lucide-react';
...
interface MatchChatProps {
  match_id: string;           // Fikstür/match ID (prop adı kullanıcı talebine uygun)
  profileId: string;
  teamName: string;
...
export default function MatchChat({
  ...
  const handleSend = useCallback(async () => {
    ...
    setNewMessage('');
    await sendMatchChatMessage(match_id, profileId, teamName, content);
  }, [newMessage, match_id, profileId, teamName]);

  // Hızlı reaksiyon gönderme
  const handleReaction = useCallback(async (emoji: string) => {
    await sendMatchReaction(match_id, profileId, teamName, emoji, currentMinute);
    setShowReactions(false);
  }, [match_id, profileId, teamName, currentMinute]);
