...
import PlayerCompareModal from '@/components/fm/PlayerCompareModal';
import { List } from 'react-window';
import { checkBigTransfer, emitEmotionalEvent } from '@/lib/fm/emotionalEvents';
import { transferWindowStatus } from '@/lib/fm/transferWindow';

// ─── Effective market value helper (pure function, used in row component) ───
// HATA-3 FIX: Always calculate from current rating to ensure freshness.
// The DB market_value column is updated weekly by cron and may be stale.
// For transfer_market listings, the `price` column overrides this (set as market_value).
function getEffectiveMarketValue(player: Player): number {
  // Transfer market listings have their price set as market_value by fetchMarketPlayers
  // Those are seller-set prices and should be respected
  const isTransferListing = (player as Record<string, unknown>).is_for_sale === true
    && (player as Record<string, unknown>).seller_id;
  if (isTransferListing && player.market_value && player.market_value > 0) {
    return player.market_value; // Seller's asking price
  }
  // For all other players, always calculate fresh from rating to avoid stale values
  return Math.round(Math.pow(player.rating || 60, 2.5) * 5000);
}

// ─── Player row data type for virtualized List ───
...
const BIG_POS_MAP: Record<string, string> = {
...
// ─── Player row component for react-window v2 List (memoized) ───
const PlayerRowComponent = React.memo(function PlayerRowComponent({ index, style, ariaAttributes, rows, onNegotiate, columnCount, comparePlayer1, onCompareSelect }: {
  ariaAttributes: { "aria-posinset": number; "aria-setsize": number; role: "listitem" };
  index: number;
  style: React.CSSProperties;
} & PlayerRowData): React.ReactElement | null {
  const row = rows[index];
  if (!row) return null;
  return (
    <div style={style} {...ariaAttributes} className={`grid gap-4 ${columnCount === 3 ? 'grid-cols-3' : columnCount === 2 ? 'grid-cols-2' : 'grid-cols-1'}`}>
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex-1 py-3 bg-white/5 group-hover:bg-white group-hover:text-black rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2"
            >
            ...
            >
              {comparePlayer1?.id === player.id ? '✓ Seçildi' : '⚖️'}
            </button>
          </div>
        </motion.div>
      ))}
...
export default React.memo(function MarketTab() {
  ...
  const windowStatus = transferWindowStatus((profile as any)?.current_day);
  ...
  // ── Karşılaştırma state ──
  const [comparePlayer1, setComparePlayer1] = useState<Player | null>(null);
  const [comparePlayer2, setComparePlayer2] = useState<Player | null>(null);
  const [showCompare, setShowCompare] = useState(false);

  // ── Responsive column count for virtualized player grid ──
  ...
  // ── HATA-2 FIX: Realtime subscription for new transfer market listings ──
  // Yoğun transfer durumunda sayısız refetch'i önlemek için debounce uygulanır.
  // Her INSERT/UPDATE/DELETE event'inde 300ms beklenir, yeni event gelirse
  // süre sıfırlanır — böylece toplu değişikliklerde tek bir fetch yapılır.
  useEffect(() => {
    if (!isSupabaseConfigured() || !profile) return;
    const supabase = getSupabase();
    if (!supabase) return;

    // Realtime event'leri için debounce zamanlayıcı
    ...
    const channel = supabase
      .channel('transfer_market_changes')
      .on(
        'postgres_changes',
        {
          event: 'INSERT',
        ...
        () => {
          // Yeni ilan eklendiğında debounce ile yenile
          debouncedFetch();
        }
      )
      .on(
        ...
        () => {
          // İlan güncellendiğinde debounce ile yenile
          debouncedFetch();
        }
      )
      .on(
        ...
        () => {
          // İlan silindiğinde debounce ile yenile
          debouncedFetch();
        }
      )
      .subscribe();
  ...
  const availablePlayers = useMemo(() => {
    ...
    return unique.filter((p: Player) => {
      ...
      if (positionFilter !== 'ALL') {
        ...
        const filterGroup = BIG_POS_MAP[positionFilter];
        if (filterGroup) {
          const playerBigPos = BIG_POS_MAP[playerPos] || playerPos;
  ...
  // Group players into rows for virtualized rendering
  const playerRows = useMemo(() => {
    const rows: Player[][] = [];
    for (let i = 0; i < availablePlayers.length; i += columnCount) {
      rows.push(availablePlayers.slice(i, i + columnCount));
    }
    return rows;
  }, [availablePlayers, columnCount]);
  ...
  // ── Karşılaştırma seçim handler ──
  const handleCompareSelect = useCallback((player: Player) => {
  ...
  const handleOpenNegotiation = useCallback((player: Player) => {
  ...
  const handleNegotiate = async () => {
    ...
    try {
      ...
      if (res.success) {
        // Büyük transfer duygusal olayı (10M+ Euro)
        try {
          if (profile) {
            const bigTransferEvent = checkBigTransfer(effectiveOffer, negotiatingPlayer.name, profile, 10_000_000);
            if (bigTransferEvent) {
              emitEmotionalEvent(bigTransferEvent);
            }
          }
        } catch (err) {
          console.warn('[MarketTab] Big transfer event failed:', err);
        }
  ...
  // Sponsors — module-level constant (no re-creation on every render)
  const availableSponsors = AVAILABLE_SPONSORS;

  // ─── Memoized offer filters (avoid repeated .filter() calls) ───
  const pendingOffers = useMemo(() => myOffers.filter((o: any) => o.status === 'pending'), [myOffers]);
  const resolvedOffers = useMemo(() => myOffers.filter((o: any) => o.status !== 'pending'), [myOffers]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div>
            <h2 className="text-lg sm:text-2xl font-black italic uppercase tracking-tighter text-white">YÖNETİM & TİCARET</h2>
            <p className="text-[10px] text-white/40 font-black uppercase tracking-widest">Transferler ve Sponsorluk Anlaşmaları</p>
            <div className={`text-xs px-3 py-1.5 rounded-full border ${windowStatus.isOpen ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-red-500/10 border-red-500/20 text-red-400'}`}>
      ...
      {activeSubTab === 'transfers' ? (
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input 
                ...
                className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 sm:py-4 pl-12 pr-4 text-xs font-black uppercase tracking-widest focus:outline-none focus:border-white/30 transition-all"
              />
            </div>
          ...
          {/* Player Grid — Virtualized for performance */}
          {marketLoading ? (
              ...
              <List<PlayerRowData>
                rowComponent={PlayerRowComponent}
                rowCount={playerRows.length}
                rowHeight={240}
                rowProps={{ rows: playerRows, onNegotiate: handleOpenNegotiation, columnCount, comparePlayer1, onCompareSelect: handleCompareSelect }}
                overscanCount={5}
                style={{ height: Math.min(playerRows.length * 240, 800) }}
              />
            )}
        </div>
                                ...
                                onClick={async () => {
                                  ...
                                  try {
                                    ...
                                    if (data.success) {
                                      ...
                                      toastError(data.error || 'Hata oluştu');
                                    }
                                  } catch (err) {
      ...
      {/* ── Karşılaştırma sticky banner ── */}
      {comparePlayer1 && !showCompare && (
        <div className="sticky bottom-0 bg-zinc-900 border-t border-amber-500/20 p-3 flex items-center justify-between">
      ...
      {showCompare && (
        <PlayerCompareModal
          ...
          onClose={() => { setShowCompare(false); setComparePlayer1(null); setComparePlayer2(null); }}
        />
      )}
