...
export default function RefereeSection() {
  ...
  const fetchReferees = useCallback(async () => {
    ...
    try {
      ...
      console.error('[RefereeSection] Fetch error:', err);
    } finally {
      setLoading(false);
    }
  }, [profile?.id]);
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {referees.map((ref, i) => {
          const personality = ref.personality || 'dengeci';
          const colors = PERSONALITY_COLORS[personality] || PERSONALITY_COLORS.dengeci;
          const description = PERSONALITY_DESCRIPTIONS[personality] || PERSONALITY_DESCRIPTIONS.dengeci;
