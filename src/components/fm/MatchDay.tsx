  ...
  const getCycleStatus = useCallback(() => {
    ...
    if (isWeekday) {
      if (currentHour < 10 || (currentHour === 10 && currentMinute < 0)) {
        return { phase: 'IDLE' as const, nextEventTime: '10:00', countDownMinutes: 0, isTrainingWindow: false };
      }
      if ((currentHour === 10 || currentHour === 16) && currentHour < 12) {
        return { phase: 'PRE_MATCH' as const, nextEventTime: currentHour < 12 ? '12:00' : '18:00', countDownMinutes: (currentHour < 12 ? (12 - currentHour) * 60 - currentMinute : (18 - currentHour) * 60 - currentMinute), isTrainingWindow: false };
      }
      if (currentHour >= 12 && currentHour < 18) {
        return { phase: 'PRE_MATCH' as const, nextEventTime: '18:00', countDownMinutes: (17 - currentHour) * 60 + (60 - currentMinute), isTrainingWindow: false };
      }
    }
    return { phase: 'IDLE' as const, nextEventTime: 'Pazartesi 12:00', countDownMinutes: 0, isTrainingWindow: false };
  }, []);
  ...
  if (!isActive && (cycleStatus.phase === 'IDLE' || cycleStatus.phase === 'POST_MATCH' || cycleStatus.phase === 'TRAINING_WINDOW')) {
    return (
      <div className="flex flex-col h-full min-h-[600px] bg-black/80 backdrop-blur-md border border-white/5">
        ...
        {/* Maç sayfasına yönlendir — canlı maç artık /match/[id] sayfasında */}
