...
const REFEREE_LABELS: Record<string, { emoji: string; label: string; color: string }> = {
  strict: { emoji: '🟥', label: 'Katılcı', color: 'text-red-400 bg-red-500/10 border-red-500/20' },
  balanced: { emoji: '⚖️', label: 'Dengeci', color: 'text-yellow-400 bg-yellow-500/10 border-yellow-500/20' },
  lenient: { emoji: '🤝', label: 'Hoşgörülü', color: 'text-green-400 bg-green-500/10 border-green-500/20' },
  home_bias: { emoji: '🏠', label: 'Ev Sahibi', color: 'text-blue-400 bg-blue-500/10 border-blue-500/20' },
  volatile: { emoji: '🎲', label: 'Değişken', color: 'text-purple-400 bg-purple-500/10 border-purple-500/20' },
  var_lover: { emoji: '📺', label: 'VAR Sever', color: 'text-cyan-400 bg-cyan-500/10 border-cyan-500/20' },
...
// ─── Opponent Preview Popup ─────────────────────────────────
function OpponentPreview({ teamName, onClose }: { teamName: string; onClose: () => void }) {
  const [data, setData] = useState<{ avgRating: number; topPlayer: string; form: string } | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data: players } = await supabase
        .from('players')
        .select('name, rating, form_rating, match_ratings')
      ...
      const avgRating = Math.round(players.reduce((s, p) => s + (p.rating || 0), 0) / players.length);
      const topPlayer = players[0]?.name?.split(' ').pop() || players[0]?.name || '?';

      const recentRatings = players
        .flatMap(p => (Array.isArray(p.match_ratings) ? p.match_ratings.slice(-3) : []))
        .filter(Boolean);
      const avgForm = recentRatings.length
        ? recentRatings.reduce((a: number, b: number) => a + b, 0) / recentRatings.length
        : 6.0;
      const form = avgForm > 7 ? 'İyi Form' : avgForm > 5.5 ? 'Orta Form' : 'Kötü Form';

      setData({ avgRating, topPlayer, form });
    })();
  }, [teamName]);
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0 }}
      className="absolute z-50 bg-zinc-900 border border-white/15 rounded-xl p-4 shadow-2xl w-52"
      style={{ top: '100%', left: 0 }}
    >
      <div className="flex items-center justify-between mb-3">
        <p className="text-[10px] font-black text-white">{teamName}</p>
        <button onClick={onClose} className="text-white/30 hover:text-white text-xs">✕</button>
      </div>
      {data ? (
        <div className="space-y-2">
          <div className="flex justify-between text-[10px]">
            <span className="text-white/40">Ort. OVR</span>
            <span className="font-black text-amber-400">{data.avgRating}</span>
          </div>
          <div className="flex justify-between text-[10px]">
            <span className="text-white/40">Anahtar Oyuncu</span>
            <span className="font-black text-white">{data.topPlayer}</span>
          </div>
          <div className="flex justify-between text-[10px]">
            <span className="text-white/40">Form</span>
            <span className={`font-black ${
              data.form === 'İyi Form' ? 'text-emerald-400' :
              data.form === 'Orta Form' ? 'text-amber-400' : 'text-red-400'
            }`}>{data.form}</span>
          </div>
        </div>
      ) : (
        <p className="text-[9px] text-white/30">Yükleniyor...</p>
      )}
    </motion.div>
  );
}
                      ...
                      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="relative">
                            <span
                              className="cursor-pointer hover:text-amber-400 transition-colors text-[9px] font-bold text-white/30"
                              onClick={() => setPreviewTeam(previewTeam === nextOpponentName ? null : nextOpponentName)}
                            >
                              — {nextOpponentName}
                            </span>
                            <AnimatePresence>
                              {previewTeam === nextOpponentName && (
                                <OpponentPreview
                                  teamName={nextOpponentName}
