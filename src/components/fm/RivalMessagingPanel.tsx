...
interface RivalMessagingPanelProps {
  userId: string;
  userName: string;
  teamName: string;
}
...
export default function RivalMessagingPanel({ userId, userName, teamName }: RivalMessagingPanelProps) {
  ...
  const [presences, setPresences] = useState<Map<string, ManagerPresence>>(new Map());
  ...
  const loadConversations = useCallback(async () => {
    ...
    if (convs) {
      ...
      if (ids.length > 0) {
        ...
        if (pList) {
          const map = new Map<string, ManagerPresence>();
  ...
  useEffect(() => {
    if (directMessageRecipient) {
      setTimeout(async () => {
        ...
        const { conversation } = await getOrCreateConversation(userId, otherId, otherName);
        if (conversation) {
  ...
  }, [directMessageRecipient, setDirectMessageRecipient, userId, conversations, openConversation]);
  ...
  const handleSend = useCallback(async () => {
    ...
    setIsSending(true);
    try {
      const { message } = await sendDirectMessage(activeConversation.id, userId, inputText, selectedCategory);
      if (message) {
        setMessages(prev => {
  ...
  }, [inputText, activeConversation, userId, selectedCategory, isSending, loadConversations, loadUnreadCount]);
    ...
    return (
      ...
      >
        <div className={`max-w-[85%] relative ${
          ...
          {/* Delete button on hover (only own messages) */}
          {isMine && (
            <button
                    ...
                    <div className="flex-1 overflow-y-auto space-y-1">
                      ...
                      {searchQuery.length < 2 && (
                        <div className="flex flex-col items-center justify-center py-8 opacity-20">
                          <Users size={24} />
                          <p className="text-[10px] mt-2">Aramak için en az 2 karakter</p>
                        </div>
                      )}
                    </div>
                  </motion.div>
                  ...
                  >
                    ...
                    <div className="p-2.5 bg-zinc-900/50 border-t border-white/5 shrink-0">
                      ...
                      <div className="flex items-center gap-1.5">
                        <button
                          onClick={() => { setShowCategories(!showCategories); setShowQuickReplies(false); }}
                          className={`shrink-0 p-2 rounded-lg transition-all ${
                            showCategories ? 'bg-red-500/10 text-red-400' : 'hover:bg-white/5 text-white/20 hover:text-white/40'
                          }`}
                        ...
                        <button
                          onClick={() => { setShowQuickReplies(!showQuickReplies); setShowCategories(false); }}
                          className={`shrink-0 p-2 rounded-lg transition-all ${
                            showQuickReplies ? 'bg-blue-500/10 text-blue-400' : 'hover:bg-white/5 text-white/20 hover:text-white/40'
                          }`}
                        ...
                        <button
                          ...
                          className="shrink-0 w-9 h-9 bg-team-primary hover:bg-team-primary-hover disabled:bg-white/5 disabled:text-white/10 rounded-xl flex items-center justify-center transition-all active:scale-95"
                        >
