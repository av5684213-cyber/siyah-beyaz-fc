...
import { Trophy, ArrowUpCircle, ArrowDownCircle, MinusCircle, RefreshCw, Shield, ChevronRight, Users, Search, Bot, AlertTriangle } from 'lucide-react';
...
export default React.memo(function LeagueStandings({ isAdmin }: { isAdmin?: boolean }) {
  ...
  // Ref for auto-scrolling to user's team row
  const userRowRef = useRef<HTMLDivElement>(null);

  // ── standings must be defined BEFORE the useEffect that references it (TDZ fix) ──
  const standings = useMemo(() => data?.standings || [], [data?.standings]);

  // Auto-scroll to user's team row when standings change
  useEffect(() => {
  ...
  // ── TDZ fix: activeTierLeagues MUST be useMemo (not plain const) ──
  // After SWC minification (const→let + let-chain merging), plain const
  // computed values can end up in a let chain where the initializer
  // references variables that are still in the TDZ of that same chain.
  // Using useMemo ensures the value is computed at a well-defined point
  // with explicit dependencies, matching the fix in match/[id]/page.tsx.
  const activeTierLeagues = useMemo(() =>
    tierMap[leagues.find(l => String(l.id) === String(effectiveActiveLeague))?.tier || 1] || [],
    [tierMap, leagues, effectiveActiveLeague]
  );
  ...
  // ── TDZ fix: currentLeague & colors must also be useMemo ──
  const currentLeague = useMemo(() =>
    leagues.find(l => String(l.id) === String(effectiveActiveLeague)),
    [leagues, effectiveActiveLeague]
  );
  const colors = useMemo(() =>
    LEAGUE_COLORS[currentLeague?.tier || 1] || LEAGUE_COLORS[1],
    [currentLeague]
  );
