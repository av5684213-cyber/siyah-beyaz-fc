...
export default function YouthAcademyTab({
  ...
  const handleIntake = useCallback(() => {
    ...
    if (seasonIntakeUsed) {
      toast({ title: '⚠️ Alım Yapılamaz', description: 'Bu sezonun alımı zaten yapıldı!', variant: 'destructive' });
      setShowIntakeConfirm(false);
      return;
    }
    ...
    if (currentCredits < 10) {
      toast({ title: '⚠️ Yetersiz Kredi', description: '10 Kredi gerekli.', variant: 'destructive' });
      setShowIntakeConfirm(false);
      return;
    }
    // Deduct 10 credits
  ...
  const handlePromote = useCallback((player: YouthPlayer) => {
    ...
    toast({
      title: `🎉 ${player.name?.split(' ').pop()} Birinci Takıma Katıldı!`,
      description: `${player.age} yaşında, ${player.rating} OVR. Geleceğin yıldızı sizi bekliyor!`,
    });
  }, [onPromotePlayer]);
                        ...
                        <div className="flex items-center gap-2.5">
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5">
                              ...
                              {(() => {
                                const isReady = (player.rating || 0) >= 60 && (player.age || 0) >= 18;
                                ...
                                return (
                                  <span className={`text-[10px] font-black uppercase px-1.5 py-0.5 rounded border ${
                                    ...
                                    {isSuperTalent ? '⭐ Süper Yetenek' : '✅ Terfi Hazır'}
                                  </span>
                                );
                              })()}
                            </div>
              ...
              <div className="relative z-10">
                ...
                <div className="bg-black/30 border border-white/[0.04] rounded-xl p-4 mb-6 space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[12px] font-mono font-bold text-white">15-18</span>
                  </div>
                  <div className="flex items-center justify-between">
