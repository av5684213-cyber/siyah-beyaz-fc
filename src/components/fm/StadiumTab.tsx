...
const ACADEMY_STEPS: AcademyStep[] = [
  ...
  { level: 9, name: "Yüksek Performans Lab.", buff: "Özel Yetenek", desc: "Oyuncuların '%15 şansla' wonderkid doğma şansı olur.", cost: 300000000 },
  { level: 10, name: "Yıldız Fabrikası", buff: "Wonderkid Üssü", desc: "En yüksek seviye arketipli oyuncular üretilir, wonderkid şansı %20.", cost: 1000000000 },
];
...
): { weeklyGain: number; paybackWeeks: number } {
  const gainByType: Record<string, number> = {
    'capacity':    currentWeeklyRevenue * 0.12,
    'vip':         currentWeeklyRevenue * 0.08,
    'hospitality': currentWeeklyRevenue * 0.06,
    'training':    0,
    'medical':     currentWeeklyRevenue * 0.02,
    'pitch':       currentWeeklyRevenue * 0.03,
  };
  const weeklyGain = Math.round(gainByType[upgradeType] || currentWeeklyRevenue * 0.05);
  const paybackWeeks = weeklyGain > 0 ? Math.ceil(upgradeCost / weeklyGain) : 999;
  return { weeklyGain, paybackWeeks };
}
...
export default function StadiumTab() {
  ...
  const MAX_TICKET_BY_TIER: Record<number, number> = {
    1: 120,   // Süper Lig: max 120€
    2: 90,    // 1. Lig: max 90€
    3: 60,    // 2. Lig: max 60€
    4: 40,    // 3. Lig: max 40€
  };
  const tier = profile?.league_tier || 4;
  const maxPrice = MAX_TICKET_BY_TIER[tier] || 40;

  const [ticketPrice, setTicketPrice] = useState(Math.min(maxPrice, profile?.ticket_price || 20));
  ...
  const [dbFacilityLevels, setDbFacilityLevels] = useState<Record<string, number>>({});
  ...
  // ── BUG D5 FIX: Sync ticketPrice when profile changes (tier change, DB reload, etc.) ──
  // When league_tier changes (promotion/relegation), cap the displayed price to the new tier's max.
  // When ticket_price changes from the DB (e.g., after moveTeamToLeague capped it), update the display.
  // This does NOT auto-save to DB — only explicit user input triggers a DB save.
  useEffect(() => {
    const currentProfilePrice = profile?.ticket_price ?? 20;
    const cappedPrice = Math.min(maxPrice, currentProfilePrice);
    setTicketPrice(cappedPrice);
  }, [profile?.ticket_price, profile?.league_tier, maxPrice]);
    ...
    const fetchFacilities = async () => {
      try {
        ...
        if (data.facilities && Array.isArray(data.facilities)) {
          ...
          const mergedUpgrades = { ...profile.stadium_upgrades, ...levels };
          if (JSON.stringify(mergedUpgrades) !== JSON.stringify(profile.stadium_upgrades)) {
            setProfile({ ...profile, stadium_upgrades: mergedUpgrades });
          }
        }
      } catch (err) {
  ...
  const handleUpdateTicketPrice = async (price: number) => {
    if (!profile) return;
    const finalPrice = Math.min(maxPrice, Math.max(0, price));
    setTicketPrice(finalPrice);
    setProfile({ ...profile, ticket_price: finalPrice });
    // Hemen veritabanına kaydet (auto-save'e bırakma)
    if (isSupabaseConfigured()) {
      const supabase = getSupabase();
      if (supabase) {
        try {
          await supabase.rpc('rpc_update_profile', { p_profile_id: profile.id, p_updates: { ticket_price: finalPrice } });
        } catch (err) {
          console.warn('[StadiumTab] Ticket price DB save error:', err);
        }
      }
    }
  };

  const handleStartUpgrade = async (id: string, cost: number, currentLvl: number) => {
    if (!profile) return;
        ...
        <div className="bg-zinc-900 border border-white/5 rounded-2xl sm:rounded-[2rem] p-5 sm:p-8 flex flex-col justify-between relative group overflow-hidden">
          ...
          <div>
            ...
            <div className="flex flex-col gap-1">
               ...
               <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                  <div className="h-full bg-amber-500 transition-all duration-500" style={{ width: `${(1 - ticketPrice / maxPrice) * 100}%` }} />
               </div>
            </div>
          </div>
        </div>
                ...
                {!isMax && (
                  ...
                  {(() => {
                    const roi = calculateUpgradeROI(
                      ...
                      level,
                      cost,
                      profile?.last_weekly_income || 50000
                    );
