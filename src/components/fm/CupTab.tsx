...
function BracketMatchCard({ match, teamName }: { match: BracketMatch; teamName: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="text-center font-mono font-black my-1">
        {match.homeScore != null ? (
          <a href={`/match/${match.id}`} className="text-emerald-400 hover:text-emerald-300 transition-colors">
            {scoreText}
          </a>
        ) : (
          <span className="text-white/30">{scoreText}</span>
        )}
      </div>
      <div
