    ...
    <motion.div key="dash" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
       ...
       {squadHealth && (
         <div className="bg-white/[0.03] border border-white/8 rounded-xl p-4 flex items-center justify-between gap-3">
           <p className="text-[9px] uppercase tracking-widest text-white/25 shrink-0">Kadro Durumu</p>
           <div className="flex gap-4">
             <div className="text-center">
               <p className="text-sm font-black text-emerald-400">{squadHealth.fit}</p>
               <p className="text-[8px] text-white/30">Fit</p>
             </div>
             <div className="text-center">
               <p className="text-sm font-black text-amber-400">{squadHealth.tired}</p>
               <p className="text-[8px] text-white/30">Yorgun</p>
             </div>
             <div className="text-center">
               <p className="text-sm font-black text-red-400">{squadHealth.injured}</p>
               <p className="text-[8px] text-white/30">Sakat</p>
             </div>
             {squadHealth.lowMoral > 0 && (
               <div className="text-center">
                 <p className="text-sm font-black text-purple-400">{squadHealth.lowMoral}</p>
                 <p className="text-[8px] text-white/30">Moralsiz</p>
               </div>
             )}
           </div>
         </div>
           ...
           <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
             <button 
               onClick={() => {
                 if (profile) {
                   ...
                   toast.success('Hesaba 100M € eklendi!');
                 }
               }}
             ...
             <button 
               onClick={() => {
                 ...
                 toast.success('Tüm kadro fitness %100 yapıldı!');
               }}
               className="bg-black/40 border border-red-600/20 p-4 rounded-2xl flex flex-col items-center gap-2 hover:bg-red-600/20 transition-all text-xs font-black uppercase tracking-widest text-red-400"
             >
             ...
             <button
               onClick={async () => {
                 try {
                   ...
                   const data = await res.json();
                   toast.success('Haftalık evrim: ' + (data.updated || 0) + '/' + (data.total || 0) + ' oyuncu güncellendi!');
                 } catch (err) { toast.error('Evrim hatası: ' + err); }
               }}
               className="bg-black/40 border border-red-600/20 p-4 rounded-2xl flex flex-col items-center gap-2 hover:bg-red-600/20 transition-all text-xs font-black uppercase tracking-widest text-red-400"
             >
             ...
             <button 
               onClick={() => {
                 ...
                 toast.success('Tüm kadroya +5 genel yetenek eklendi!');
               }}
               className="bg-black/40 border border-red-600/20 p-4 rounded-2xl flex flex-col items-center gap-2 hover:bg-red-600/20 transition-all text-xs font-black uppercase tracking-widest text-red-400"
             >
       ...
       {lastResult && (
         <div className={
           lastResult.result === 'W' ? 'border border-emerald-500/20 bg-emerald-500/5 rounded-xl p-4 flex items-center justify-between' :
           lastResult.result === 'D' ? 'border border-blue-500/20 bg-blue-500/5 rounded-xl p-4 flex items-center justify-between' :
           'border border-red-500/20 bg-red-500/5 rounded-xl p-4 flex items-center justify-between'
         }>
           <div>
             <p className={
               lastResult.result === 'W' ? 'text-[9px] uppercase tracking-widest text-emerald-400/60' :
               lastResult.result === 'D' ? 'text-[9px] uppercase tracking-widest text-blue-400/60' :
               'text-[9px] uppercase tracking-widest text-red-400/60'
             }>Son Maç</p>
             <p className={
               lastResult.result === 'W' ? 'text-[11px] font-black text-emerald-400' :
               lastResult.result === 'D' ? 'text-[11px] font-black text-blue-400' :
               'text-[11px] font-black text-red-400'
             }>
               {lastResult.result === 'W' ? 'GALİBİYET' : lastResult.result === 'D' ? 'BERABERLİK' : 'MAĞLUBİYET'}
             </p>
           </div>
           <p className={
             lastResult.result === 'W' ? 'text-3xl font-black tabular-nums text-emerald-400' :
             lastResult.result === 'D' ? 'text-3xl font-black tabular-nums text-blue-400' :
             'text-3xl font-black tabular-nums text-red-400'
           }>{lastResult.score}</p>
         </div>
       )}
