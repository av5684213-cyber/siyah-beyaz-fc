...
import { toTitleCase, getPosDotColor } from '@/lib/fm/ui-helpers';
...
export default function TacticLab({ onClose, squad }: TacticLabProps) {
    ...
    const loadSession = async () => {
      try {
        ...
        const { data, error } = await supabase
          .from('lab_sessions')
          .select('team_a, team_b, selected_formation, settings')
          .eq('user_id', user.id)
          .maybeSingle();
  ...
  const runBatchSim = useCallback(async (count: number = 5) => {
    if (squadAMembers.length === 0 || squadBMembers.length === 0) {
      sonnerToast.error("Her iki takımda da en az 1 oyuncu olmalıdır.");
      return;
    }
    ...
    for (let i = 0; i < count; i++) {
      const res = await runUnifiedMatch(squadAMembers, squadBMembers, {
        activeTactic: {
    ...
    >
      ...
      >
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          ...
          <div className="w-full md:w-80 border-r border-white/5 flex flex-col bg-black/20 overflow-y-auto">
          ...
          <div className="flex-1 bg-[radial-gradient(circle_at_center,_rgba(37,99,235,0.1)_0%,_transparent_70%)] relative overflow-hidden flex flex-col pt-20">
              ...
              <div className="flex-1 p-4 md:p-8 flex flex-col items-center justify-start overflow-hidden">
                    <div className="relative w-full aspect-[4/3] max-h-[400px] md:max-h-[600px] border-4 border-white/5 rounded-3xl bg-emerald-950/20 overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.5)]">
          ...
          <div className="w-full md:w-96 border-l border-white/5 flex flex-col bg-zinc-950 overflow-y-auto">
             <div className="p-8 space-y-8">
                ...
                <div className="mt-auto pt-8">
                   <button 
                     onClick={() => toast({ title: '✅ Taktik Kopyalandı', description: 'Ana taktik panelinize uygulandı.' })}
