...
export function FriendlyMatchTab() {
  ...
  const handleJoinPriorityQueue = async () => {
    ...
    try {
      if (isSupabaseConfigured()) {
        ...
        await supabase.rpc('rpc_update_profile', { p_profile_id: profile.id, p_updates: { credits: newCredits } });
        setProfile((prev: any) => {
          if (!prev) return prev;
  ...
  // Maç sonrası kondisyon düşüşü uygula
  const drainCond = useCallback(async (playerIds: string[], drain: number) => {
    if (!isSupabaseConfigured()) return;
    const supabase = getSupabase();
    if (!supabase) return;
    for (const pid of playerIds) {
      const { data: p } = await supabase.from('players').select('cond').eq('id', pid).maybeSingle();
      if (p) {
        await supabase.rpc('rpc_save_training_result', { p_profile_id: profile.id, p_player_id: pid, p_updates: { cond: Math.max(10, (p.cond || 80) - drain) } });
      }
    }
  }, []);
  ...
  // Friendly match tamamlandığında kondisyon düşüşü uygula
  useEffect(() => {
    if (matchState.isFinished && matchState.isFriendly && squad.length > 0) {
      const playedIds = squad.slice(0, 11).map(p => p.id);
      drainCond(playedIds, 8); // Friendly: daha az drain
    }
  }, [matchState.isFinished, matchState.isFriendly, squad, drainCond]);
