...
export function FriendlyMatchTab() {
  ...
  const startMatch = useCallback((opponent: { name: string; squad: Player[] }) => {
  ...
  // ── Save match to friendly_matches ──
  const saveFriendlyMatch = useCallback(async (opponentId: string, homeScore: number, awayScore: number, matchResult?: Record<string, unknown>) => {
    if (!isSupabaseConfigured() || !profile) return;
    try {
      const supabase = getSupabase();
      if (!supabase) return;
  ...
  const checkForMatch = useCallback(async () => {
    ...
    try {
      ...
      if (isUserMatched) {
        ...
        const opponent = generateOpponent(opponentTeamName);

        // Save match record
        await saveFriendlyMatch(opponentId, 0, 0);
