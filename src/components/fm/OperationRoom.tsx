...
export default function OperationRoom({ trainingState, budget, onUpdateState, onDeductBudget, userId }: OperationRoomProps) {
  ...
  const handleLaunch = async (op: Operation) => {
    const { state: newState, cost, error } = manager.launchOperation(op.id, trainingState, budget);
    if (error) {
      toast({ title: '⚠️ Operasyon Başlatılamadı', description: String(error), variant: 'destructive' });
      return;
    }
    onUpdateState(newState);
    onDeductBudget(cost);

    // ── Operasyonu DB'ye kaydet (RPC ile — RLS uyumlu) ──
    if (userId) {
      const supabase = getSupabase();
      if (supabase) {
        try {
          const { error: rpcError } = await supabase.rpc('rpc_create_operation', {
            p_profile_id: userId,
            p_op_id: op.id,
            p_impact_type: (op as any).impactType || 'luck',
            p_impact_value: (op as any).impactValue || 0,
            p_target_profile_id: null,
            p_expires_at: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
          });
          if (rpcError) console.warn('[OperationRoom] RPC hatası:', rpcError.message);
        } catch (err) {
          console.warn('[OperationRoom] DB yazma hatası:', err);
        }
      }
    }
