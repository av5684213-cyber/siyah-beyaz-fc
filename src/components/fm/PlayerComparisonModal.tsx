            ...
            {filtered.map((p) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[8px] text-white/30">{p.position || p.specificPosition || '?'} · OVR {p.rating || p.ovr || '?'}</div>
                </div>
                {p.id === player.id && (
...
const ARCHETYPE_LABELS_TR: Record<string, string> = {
  ...
  'Kale bekçisi': 'Kale Bekçisi',
  'Süpürücü kaleci': 'Süpürücü Kaleci',
  // Defans
  'Duvar stoper': 'Duvar Stoper',
  'Topcu stoper': 'Topçu Stoper',
  'Hücumcu stoper': 'Hücumcu Stoper',
  'Hücumcu bek': 'Hücumcu Bek',
  'Defansif bek': 'Defansif Bek',
  'Dengeli bek': 'Dengeli Bek',
  // Orta saha
  'Yıkıcı orta saha': 'Yıkıcı Orta Saha',
  'Direktör orta saha': 'Direktör Orta Saha',
  'Kutu kutu orta saha': 'Kutu Kutu Orta Saha',
  'Yaratıcı orta saha': 'Yaratıcı Orta Saha',
  'Çalışkan orta saha': 'Çalışkan Orta Saha',
  // Kanat
  'Hızlı kanat': 'Hızcı Kanat',
  'Klasik kanat': 'Klasik Kanat',
  'İçe dönük kanat': 'İçe Dönük Kanat',
  // Forvet
  'Bitirici forvet': 'Bitirici Forvet',
  'Hedef adam': 'Hedef Adam',
  'Çalışkan forvet': 'Çalışkan Forvet',
  ...
  return (
    <Dialog open={open} onOpenChange={() => onClose()}>
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto bg-zinc-950 border-white/10">
        ...
        <div className="grid grid-cols-[1fr_auto_1fr] gap-2 mb-4 items-start">
          <div>
          ...
          <div className="text-white/20 text-xs pt-4">vs</div>
          <div>
              ...
              <div key={i} className={`rounded-xl border ${colorClasses} p-3`}>
                ...
                {getArchetypeLabel(p) && (
                  <div className="mt-2 pt-2 border-t border-white/10">
                    ...
                    <span className={`text-[10px] font-black ${color === 'amber' ? 'text-amber-400' : 'text-blue-400'}`}>
                      ⭐ {getArchetypeLabel(p)}
                    </span>
                  </div>
                )}
