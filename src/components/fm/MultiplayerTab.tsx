...
export function MultiplayerTab({ userId, profile, squad, onSetSquad, onSetProfile, onPlayerClick, onListingClick, teamName, isAdmin }: MultiplayerTabProps) {
  ...
  const handleBuy = async (listing: MarketListing) => {
    if (profile?.money < listing.price) {
      sonnerToast.error('Yetersiz bütçe!');
      return;
    }
  ...
  const handleLoanSubmit = async () => {
    ...
    if (!loanModalPlayer.id) {
      sonnerToast.error('Oyuncu ID bulunamadı.');
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        sonnerToast.success(`${toTitleCase(loanModalPlayer.name)} kiralık listesine çıkarıldı!`, { description: `Kiralık ücret: ${feeStr} • 10 Kredi komisyon kiracıdan alınacak.` });
        ...
        sonnerToast.error(data.error || 'Kiralık listesine çıkarılamadı.' + debugInfo);
      }
    } catch (err) {
    ...
    if (confirm(confirmMsg)) {
      ...
      if (result.success) {
        if (result.autoWin) {
          ...
          onSetSquad(newSquad);
          onSetProfile({ ...profile, money: profile.money - result.price });
          sonnerToast.success('Maksimum bedel ödendi! Oyuncu kadronuza katıldı.');
        } else {
          toast({ title: '📤 Teklif İletildi', description: 'Satıcı teklifinizi değerlendirecek.' });
        }
        fetchData();
      } else {
  ...
  const handleCancelAuction = async (listingId: string) => {
    ...
    try {
      ...
      if (result.success) {
        sonnerToast.success('Artırma iptal edildi.');
        fetchData();
      } else {
        sonnerToast.error(result.error || 'İptal başarısız.');
      }
    } catch (err) {
      sonnerToast.error('Bir hata oluştu.');
    } finally {
                  ...
                  {sortedAndFilteredListings.map((listing) => {
                    ...
                    return (
                      <div
                        ...
                        className="bg-white/5 rounded-xl p-3 border border-white/10 active:bg-white/10 transition-colors cursor-pointer"
                      >
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {listing.is_auction && <AuctionTimer expiresAt={listing.expires_at} />}
                        </div>
                      </div>
                    );
                  })}
                          ...
                          <button
                            onClick={async () => {
                              ...
                              try {
                                ...
                                const data = await res.json();
                                if (data.gaveUp) {
                                  sonnerToast.success(`Vazgeçildi. ${formatCurrency(data.penalty)} tazminat ödediniz.`);
                                  onSetProfile({ ...profile, money: profile.money - data.penalty });
                                  fetchData();
                                } else {
                                  sonnerToast.error(data.reason || 'İşlem başarısız.');
                                }
                              } catch (err) {
                                sonnerToast.error('Bir hata oluştu.');
                              }
        ...
        <ContractOfferModal
          ...
          onOfferResult={(result) => {
            if (result.accepted) {
              ...
              sonnerToast.success('Sözleşme başarıyla imzalandı! Oyuncu kadronuza katıldı.');
              fetchData();
            }
