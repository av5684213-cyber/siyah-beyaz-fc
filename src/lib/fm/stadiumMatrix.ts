...
/**
 * Supabase user_facilities tablosundan ev sahibi takımın stadyum seviyelerini çeker.
 * Eğer tablo yoksa veya veri yoksa profiles.stadium_upgrades JSON'ını dener.
 */
export async function fetchStadiumLevels(profileId: string): Promise<Record<string, number>> {
  try {
    ...
    if (profile?.stadium_upgrades) {
      ...
      return profile.stadium_upgrades as Record<string, number>;
    }
