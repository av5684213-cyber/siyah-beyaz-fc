...
import { getStadiumTicketRevenueMultiplier, getVIPRevenuePerMatch, getStoreDailyRevenue, getMediaSponsorMultiplier } from './stadiumMatrix';
import { getInflationFactor, applyInflationToRevenue } from './inflation';
import { calculateDemandAttendance, MAX_TICKET_PRICE } from './finance/ticketDemand';
...
export function calculateWeeklyRevenue(
  ...
  tier: number = 1,
  currentDay?: number,       // Enflasyon hesabı için oyun günü
): RevenueBreakdown {
  ...
  // Enflasyon çarpanı: gelir kaynaklarını enflasyona endeksle
  const inflationFactor = currentDay ? getInflationFactor(currentDay) : 1;
  ...
  if (isHomeMatch) {
    ...
    const upgrades = (profile as any).stadium_upgrades || {};
    const stadiumLevel = upgrades['capacity'] || 0;
    const ticketMultiplier = getStadiumTicketRevenueMultiplier(stadiumLevel);
    ...
    const vipRevenue = getVIPRevenuePerMatch(vipLevel);
    ...
    // Merchandise per home match ≈ reputation-dependent (enflasyon endeksli)
    const merchBase = currentDay ? applyInflationToRevenue(15000, currentDay) : 15000;
    ...
    // Parking & F&B (enflasyon endeksli)
    const parkingFbPerPerson = currentDay ? applyInflationToRevenue(12, currentDay) : 12;
    const parkingFbRevenue = Math.round(actualAttendance * parkingFbPerPerson);
    matchdaySources.push({
      id: 'parking_fb',
  ...
  // Away match – minimal merch at stadium shop (enflasyon endeksli)
  if (!isHomeMatch) {
    const awayShopBase = currentDay ? applyInflationToRevenue(5000, currentDay) : 5000;
    matchdaySources.push({
      id: 'shop_sales_away',
      ...
      frequency: 'weekly',
      isVariable: true,
      calculation: 'sabit mağaza cirosu (deplasman haftası, enflasyon ayarlı)',
    });
  }
  ...
  // Sponsor weekly payouts (with media sponsor multiplier + enflasyon)
  const existingSponsors = (profile.sponsors ?? []) as unknown as Sponsor[];
  const mediaLevel = ((profile as any).stadium_upgrades || {})['media'] || 0;
  const mediaMultiplier = getMediaSponsorMultiplier(mediaLevel);
  let weeklySponsorIncome = 0;
  for (const sp of existingSponsors) {
    weeklySponsorIncome += Math.round(sp.weeklyPayout * mediaMultiplier * inflationFactor);
  }
  ...
  // Kit sales – reputation based (enflasyon endeksli)
  const kitBase = currentDay ? applyInflationToRevenue(8000, currentDay) : 8000;
  const kitWeekly = Math.round(kitBase * (profile.reputation / 50));
  commercialSources.push({
    id: 'kit_sales',
  ...
  // Commercial partner income based on reputation (enflasyon endeksli)
  const partnerBase = currentDay ? applyInflationToRevenue(200, currentDay) : 200;
  const partnerIncome = Math.round(profile.reputation * partnerBase);
  if (partnerIncome > 0) {
    commercialSources.push({
  ...
  const storeLevel = ((profile as any).stadium_upgrades || {})['store'] || 0;
  const storeDailyRevenue = getStoreDailyRevenue(storeLevel);
  ...
  // ── Broadcast Revenue (enflasyon endeksli) ────────────────────────
  const broadcastSources: RevenueSource[] = [];

  const tierMultiplier = { 1: 1, 2: 0.45, 3: 0.18, 4: 0.15 }[tier] ?? 0.15;
  const baseWeeklyBroadcast = currentDay
    ? applyInflationToRevenue(Math.round(350_000 * tierMultiplier), currentDay)
    : Math.round(350_000 * tierMultiplier);
  ...
  // Per-match broadcast bonus (enflasyon endeksli)
  if (isHomeMatch) {
    const matchBonus = currentDay
      ? applyInflationToRevenue(Math.round(120_000 * tierMultiplier), currentDay)
      : Math.round(120_000 * tierMultiplier);
    broadcastSources.push({
  ...
  // ── Prize Revenue (weekly amortised, enflasyon endeksli) ────────
  const prizeSources: RevenueSource[] = [];

  // League prize money based on position and tier (enflasyon endeksli)
  if (leaguePosition) {
    const tierPrizePoolBase: Record<number, number> = { 1: 50_000_000, 2: 15_000_000, 3: 5_000_000, 4: 1_500_000 };
    const poolBase = tierPrizePoolBase[tier] ?? tierPrizePoolBase[4];
    const pool = currentDay ? applyInflationToRevenue(poolBase, currentDay) : poolBase;
  ...
  // Cup progress bonus (enflasyon endeksli)
  const cupBase = currentDay ? applyInflationToRevenue(200_000, currentDay) : 200_000;
  const cupProgressBonus = Math.round(cupBase * (profile.reputation / 50));
...
export function calculateWeeklyExpenses(
  ...
  tier: number = 1,
  realStaffCost?: number,  // If provided, overrides the estimated staff cost
): ExpenseBreakdown {
  ...
  // ── Staff Costs ──────────────────────────────────────────────
  // Use real staff cost from staff table if provided, otherwise estimate
  const staffCount = 15 + stadiumLevel * 3 + trainingLevel * 2;
  const estimatedStaffCost = staffCount * 1_800;
  const staffCost = realStaffCost !== undefined && realStaffCost > 0 ? realStaffCost : estimatedStaffCost;
  operation.push({
    id: 'staff_costs',
 ...
 * Enflasyon çarpanı ile ayarlanır — gelir arttıkça tavan da artar.
 *
 * @param annualRevenue  Projected annual revenue (in currency units)
 * @param currentDay     Oyun günü (enflasyon hesabı için, opsiyonel)
 */
export function calculateWageBillLimit(annualRevenue: number, currentDay?: number): number {
  const FFP_RATIO = 0.70;
  const baseLimit = Math.round((annualRevenue * FFP_RATIO) / 52);
  // FFP tavanı zaten enflasyonlu gelirden hesaplandığı için ek çarpan gerekmez
  // Ancak currentDay verilirse, gelecek projeksiyonları için kullanılabilir
  return baseLimit;
}
...
export function buildFinancialOverview(
  ...
  options?: {
    ...
    currentDay?: number;      // Enflasyon hesabı için
  },
  ...
  const revenueBreakdown = calculateWeeklyRevenue(
    ...
    leaguePosition,
    tier,
    currentDay,    // Enflasyon entegrasyonu
  );
  ...
  const wageBillLimit = calculateWageBillLimit(projectedAnnual, currentDay);
 ...
 */
export function calculateMatchRevenueLegacy(
  stadiumLevel: number,
  leaguePosition: number,
  totalTeams: number,
  ticketPrice: number,
...
/**
 * Server-side maç geliri hesaplama.
 * Profil verisini DB'den alıp güvenli şekilde hesaplar.
 * GÖREV 2 — Para hesaplamasını server'a taşımak için.
 *
 * @param profile  DB'den çekilmiş profil (stadium_capacity, ticket_price, stadium_upgrades, reputation, money)
 * @param isHome   Ev sahibi mi?
 * @param homeScore Ev sahibi gol sayısı
 * @param awayScore Deplasman gol sayısı
 * @returns revenue (kredi) ve attendance (seyirci)
 */
export function calculateMatchRevenue(
  profile: { stadium_capacity?: number; ticket_price?: number; stadium_upgrades?: Record<string, number>; reputation?: number; money?: number },
  isHome: boolean,
  homeScore: number,
  awayScore: number,
  ...
  if (!isHome) return { revenue: 0, attendance: 0 };

  // BUG-11: Use demand curve for attendance calculation
  // Formula: attendance = capacity * (1 - (ticket_price / max_price) ^ 0.5)
  const attendance = calculateDemandAttendance(capacity, ticketPrice, MAX_TICKET_PRICE);

  // Apply stand upgrade bonus (+5% per stand level)
  const upgradeBonus = 1 + standLevel * 0.05;
  const finalAttendance = Math.min(capacity, Math.floor(attendance * upgradeBonus));

  // Revenue calculation
  const baseRevenue = finalAttendance * ticketPrice;
  const vipLevel = upgrades['vip'] || 0;
  ...
  return { revenue: Math.floor(revenue), attendance: finalAttendance };
}

// ═══════════════════════════════════════════════════════════════════
// Dinamik Piyasa: Enflasyon ve Performans Bazlı Sponsor Gelir Ayarlaması
// ═══════════════════════════════════════════════════════════════════

/**
 * Sezon ilerledikçe sponsor gelirlerini başarıya ve piyasa enflasyonuna göre ayarlar.
 * - Lig pozisyonu iyiyse sponsorlar %5-20 fazladan öder (bonus clause)
 * - Lig pozisyonu kötüyse sponsorlar %5-10 azalır (relegation clause)
 * - Enflasyon: Her sezon %3 artış (yeni sezon sponsorluklarında yansır)
 */
export function applyDynamicSponsorAdjustment(
  weeklyIncome: number,
  leaguePosition: number,
  totalTeams: number,
  reputation: number,
  ...
  // Lig pozisyonu bazlı çarpan (1. sırada %20 bonus, son sırada %10 ceza)
  const positionRatio = totalTeams > 0 ? leaguePosition / totalTeams : 0.5;
  let positionModifier = 1.0;
  if (positionRatio <= 0.15) {
    positionModifier = 1.20; // İlk %15: %20 bonus
  } else if (positionRatio <= 0.30) {
    positionModifier = 1.10; // İlk %30: %10 bonus
  } else if (positionRatio <= 0.60) {
    positionModifier = 1.00; // Orta sıra: değişiklik yok
  } else if (positionRatio <= 0.80) {
    positionModifier = 0.95; // Alt sıra: %5 ceza
  } else {
    positionModifier = 0.90; // Son %20: %10 ceza
  }

  // İtibar bazlı çarpan (yüksek itibar = marka değeri yüksek)
  const repModifier = 0.9 + (Math.min(reputation, 100) / 100) * 0.2; // 0.9 - 1.1

  // Sezon ilerlemesi bazlı enflasyon (sezon sonuna doğru %3 artış)
  const seasonProgress = Math.min(1, seasonWeek / 34);
  const inflationModifier = 1 + seasonProgress * 0.03;

  return Math.round(weeklyIncome * positionModifier * repModifier * inflationModifier);
}

/**
 * Piyasa değerini dinamik olarak hesaplar.
 * Oyuncunun son formuna, yaşına, potansiyeline ve piyasa talebine göre değer belirler.
 */
export function calculateDynamicMarketValue(
  baseRating: number,
  age: number,
  potential: number,
  formRating: number,
  ...
  demandLevel: number = 0, // 0-5 arası kaç takım ilgiliani
): number {
  // Temel değer: rating²·⁵ × 3000
  const baseValue = Math.round(Math.pow(baseRating, 2.5) * 3000);

  // Yaş faktörü: 24-28 arası prime, 30+ düşüş
  let ageFactor = 1.0;
  if (age <= 21) ageFactor = 0.7 + (age - 15) * 0.05; // Genç: düşük ama artan
  else if (age <= 28) ageFactor = 1.0; // Prime
  else if (age <= 31) ageFactor = 0.9;
  else ageFactor = Math.max(0.4, 0.9 - (age - 31) * 0.1);

  // Potansiyel faktörü: potential > rating ise premium
  const potentialPremium = potential > baseRating
    ? 1 + (potential - baseRating) * 0.02
    : 1.0;

  // Form faktörü
  const formFactor = 0.8 + (formRating / 100) * 0.4; // 0.8 - 1.2

  // Son performans faktörü (gol/asist)
  const performanceFactor = 1 + Math.min(0.3, (recentGoals * 0.05 + recentAssists * 0.03));

  // Talep faktörü: çok takım istiyorsa fiyat artar
  const demandFactor = 1 + demandLevel * 0.08; // Her talep +8%

  return Math.round(baseValue * ageFactor * potentialPremium * formFactor * performanceFactor * demandFactor);
}
