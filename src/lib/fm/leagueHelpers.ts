 ...
 *
 * Mantık sırası:
 * 1. tier=4 olan liglerde bot takım bul (is_bot=true, profile_id IS NULL)
 * 2. Bulursa → o takımı kullanıcıya ata
 * 3. Bulamazsa → yeni 4. Lig grubu oluştur (17 bot + 1 kullanıcı)
 *
  ...
  if (tier > 1) {
    ...
    if (teamCount >= 5) {
      ...
      for (const s of standings) {
        const teamId = (s.league_teams as any)?.id;
        if (teamId) standingsMap[teamId] = { points: s.points || 0, gd: s.gd || 0 };
      }
      ...
      if (winner) {
        ...
        if (targetLeague) {
          ...
          result.playoffWinner = { teamId: winner.id, teamName: winner.name };
        }
      }
    }
  }
 ...
 */
async function moveTeamToLeague(
  supabase: any,
  teamId: string,
  ...
  if (profileId) {
    ...
    if (targetLeague) {
      await supabase
        .from('profiles')
        .update({ league_name: targetLeague.name, league_tier: targetLeague.tier })
        .eq('id', profileId);
    }
  }
...
async function findOrCreateLeagueGroup(
  ...
  // 18 bot takım ekle
  const teamNames = getTeamNamesForDepartment(tier, 1);
  const botTeams = teamNames.map((name: string, i: number) => ({
    league_id: newLeague.id,
    name,
    is_npc: true,
    is_bot: true,
