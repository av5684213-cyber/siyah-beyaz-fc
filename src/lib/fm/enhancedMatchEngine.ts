...
function calculateTeamStrength(players: Player[], tactic: ActiveTactic): {
  ...
  return {
    overall: (attack * fmod.attack * OVERALL_WEIGHT_ATTACK + midfield * fmod.midfield * OVERALL_WEIGHT_MIDFIELD + defense * fmod.defense * OVERALL_WEIGHT_DEFENSE + gk * OVERALL_WEIGHT_GK) * tacticMod,
...
export function simulateEnhancedMatch(
  ...
  homeStrength.overall *= HOME_ADVANTAGE.overall;
  homeStrength.attack *= HOME_ADVANTAGE.attack;
  homeStrength.midfield *= HOME_ADVANTAGE.midfield;
  homeStrength.defense *= HOME_ADVANTAGE.defense;
  ...
  while (currentMinute <= MATCH_STRUCTURE.duration) {
    ...
    if (momentumShiftCounter >= randInt(1, 3)) {
      ...
      for (const p of activeForInjury) {
        ...
        const injuryRisk = p.currentCond < INJURY_RISK.condThresholdLow ? INJURY_RISK.low : p.currentCond < INJURY_RISK.condThresholdMid ? INJURY_RISK.mid : INJURY_RISK.base;
...
function buildActiveTacticFromDB(data: Record<string, unknown>): ActiveTactic {
  ...
  return {
    ...
    passingStyle: (data.passing_style as string) || 'Karışık',
    intensity: tempo > 70 ? 'high' : tempo < 30 ? 'low' : 'normal',
    tactic_type: (data.formation as string) || '4-4-2',
    ...
    try {
      ...
      if (isSupabaseConfigured()) {
        ...
        const { data } = await supabase
          .from('active_tactics')
          .select('*')
          .eq('profile_id', options.awayProfileId)
          .maybeSingle();
        if (data) awayTactic = buildActiveTacticFromDB(data as Record<string, unknown>);
      }
