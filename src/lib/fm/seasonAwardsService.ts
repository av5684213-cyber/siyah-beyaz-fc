 ...
 * Artık asist verisini player_career_stats tablosundan okur;
 * tablo yoksa veya veri yoksa match_events'ten fallback hesaplar.
 */
...
/**
 * Sezon ödüllerini player_achievements tablosuna kaydeder.
 * TrophyCabinetTab bu kayıtları okuyarak kullanıcının ödüllerini gösterebilir.
 */
export async function savePlayerAchievements(
  awards: SeasonAward[],
  profileId: string,
  ...
  try {
    ...
    if (entries.length > 0) {
      const { error } = await supabase
        ...
        .upsert(entries, { onConflict: 'player_id,achievement_type,season_id' });
 ...
 */
async function fetchAssistData(playerIds: string[], seasonId: string): Promise<Map<string, number>> {
  ...
  try {
    ...
    const { data: assistEvents, error: eventsError } = await supabase
      .from('match_events')
      .select('player_id')
