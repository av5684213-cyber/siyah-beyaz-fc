...
export interface CupMatch {
  id: string;
  fixture_id?: string;  // Fixtures tablosundaki gerçek ID
  round: number;
  homeTeam: string;
  awayTeam: string;
...
function nextSaturday(from?: Date): Date {
  ...
  d.setHours(18, 0, 0, 0);  // 18:00 İstanbul = 15:00 UTC (match-scheduler ile uyumlu)
  return d;
}
...
function nextWednesday(from?: Date): Date {
  ...
  d.setDate(d.getDate() + diff);
  d.setHours(18, 0, 0, 0);  // 18:00 İstanbul = 15:00 UTC
  return d;
}
...
function findRoundDate(base: Date, roundIndex: number, cupType: CupType): Date {
  ...
  if (roundIndex % 2 === 0) {
    ...
    sunday.setHours(18, 0, 0, 0);  // 18:00 İstanbul = 15:00 UTC
    return sunday;
  }
}
...
// ──────────────────────────────────────────────────────────────
//  13. GENERATE CUP FIXTURES (fixtures tablosuna yazım)
// ──────────────────────────────────────────────────────────────

/**
 * Kupa maçlarını fixtures tablosuna yazılacak formata dönüştürür.
 * Her kupa maçına uygun bir tarih atar (Çarşamba/Cumartesi günleri, 18:00).
 * Lig maçlarıyla çakışmayacak şekilde planlanır.
 */
