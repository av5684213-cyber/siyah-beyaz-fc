...
/**
 * Lig seviyesi maaş çarpanı.
 * Düşük liglerde oyuncular daha düşük maaş alır.
 * Tier 4 → rating 48 × 950 × 0.04 = 1,824 EUR/hafta (iflas önleme)
 */
const TIER_SALARY_MULTIPLIER: Record<number, number> = {
  1: 1.0,   // Tier 1 (Süper Lig): tam fiyat
  2: 0.35,  // Tier 2 (1. Lig): %35
  3: 0.12,  // Tier 3 (2. Lig): %12
  4: 0.04,  // Tier 4 (3. Lig): %4
};
 ...
 * @param leagueTier    Lig seviyesi (1-4). Düşük lig = düşük maaş çarpanı
 * @returns Haftalık maaş (Euro)
 */
export function calculatePlayerSalary(rating: number, isFreeAgent?: boolean, leagueTier?: number): number {
  const multiplier = isFreeAgent ? SALARY_MULTIPLIER_FREE_AGENT : SALARY_MULTIPLIER_NORMAL;
  const tierMod = TIER_SALARY_MULTIPLIER[leagueTier || 1] ?? 0.04;
  return Math.floor(rating * multiplier * tierMod);
}
