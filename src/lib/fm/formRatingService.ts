...
export async function updateAllFormRatings(): Promise<{
  ...
  try {
    ...
    const totalPlayersResult = await supabase
      .from('players')
...
/** Sakatlık tipleri — injuryManager ile tutarlı */
const INJURY_TYPES = [
 ...
 * Uses injuryManager.generateInjury() for severity and days calculation
 * to ensure consistency across the entire system.
 *
 * @param durationDays - Sakatlık süresi (belirtilmezse injuryManager'dan hesaplanır)
...
export function generateInjuryRecord(durationDays?: number): InjuryRecord {
  ...
  if (durationDays === undefined) {
    const { days } = generateInjuryFromManager();
    durationDays = days;
  }
  return {
