...
import { generateRandomPersonality, type PlayerPersonality } from './agentMessageEngine';
 ...
 * Dengeleme: üretim oranı azaltıldı, akademi seviyesi yüksek = kalite artar, miktar sabit kalır.
 * @param academyFacilityLevel - Akademi konutları seviyesi (0-10)
 * @returns Oyuncu sayısı (seviye 0-3 = 1, seviye 4-6 = 1-2, seviye 7-10 = 2)
 */
export function getAcademyYouthCount(academyFacilityLevel: number): number {
  if (academyFacilityLevel <= 3) return 1;
  if (academyFacilityLevel <= 6) return Math.random() < 0.5 ? 1 : 2;
  return 2;
}
...
const ACADEMY_LEVEL_CONFIG = {
  1: {
    ratingRange: [40, 48] as [number, number],
    potentialRange: [55, 62] as [number, number],
    wonderkidChance: 0.003,
    avgStatLevel: 42,
    positionDistribution: { GK: 0.05, DEF: 0.35, MID: 0.40, FWD: 0.20 },
  },
  2: {
    ratingRange: [42, 50] as [number, number],
    potentialRange: [56, 65] as [number, number],
    wonderkidChance: 0.005,
    avgStatLevel: 44,
    positionDistribution: { GK: 0.05, DEF: 0.35, MID: 0.40, FWD: 0.20 },
  },
  3: {
    ratingRange: [43, 52] as [number, number],
    potentialRange: [58, 68] as [number, number],
    wonderkidChance: 0.008,
    avgStatLevel: 47,
    positionDistribution: { GK: 0.05, DEF: 0.35, MID: 0.40, FWD: 0.20 },
  },
  4: {
    ratingRange: [44, 53] as [number, number],
    potentialRange: [60, 72] as [number, number],
    wonderkidChance: 0.012,
    avgStatLevel: 50,
    positionDistribution: { GK: 0.05, DEF: 0.35, MID: 0.40, FWD: 0.20 },
  },
  5: {
    ratingRange: [45, 54] as [number, number],
    potentialRange: [62, 74] as [number, number],
    wonderkidChance: 0.018,
    avgStatLevel: 53,
    positionDistribution: { GK: 0.05, DEF: 0.35, MID: 0.40, FWD: 0.20 },
  },
  6: {
    ratingRange: [46, 55] as [number, number],
    potentialRange: [64, 76] as [number, number],
    wonderkidChance: 0.025,
    avgStatLevel: 56,
    positionDistribution: { GK: 0.05, DEF: 0.30, MID: 0.40, FWD: 0.25 },
  },
  7: {
    ratingRange: [47, 55] as [number, number],
    potentialRange: [66, 78] as [number, number],
    wonderkidChance: 0.032,
    avgStatLevel: 58,
    positionDistribution: { GK: 0.05, DEF: 0.30, MID: 0.40, FWD: 0.25 },
  },
  8: {
    ratingRange: [48, 55] as [number, number],
    potentialRange: [68, 79] as [number, number],
    wonderkidChance: 0.04,
    avgStatLevel: 60,
    positionDistribution: { GK: 0.05, DEF: 0.28, MID: 0.40, FWD: 0.27 },
  },
  9: {
    ratingRange: [49, 55] as [number, number],
    potentialRange: [70, 80] as [number, number],
    wonderkidChance: 0.05,
    avgStatLevel: 62,
    positionDistribution: { GK: 0.05, DEF: 0.28, MID: 0.40, FWD: 0.27 },
  },
  10: {
    ratingRange: [50, 55] as [number, number],
    potentialRange: [72, 80] as [number, number],
    wonderkidChance: 0.06,
    avgStatLevel: 64,
    positionDistribution: { GK: 0.05, DEF: 0.25, MID: 0.40, FWD: 0.30 },
  },
};
...
// ─── BUG-12: Youth Age Distribution ────────────────────────────────
// Age range: 15-18 (changed from 15-21)
// Distribution: 40% age 15-16, 40% age 17, 20% age 18
// Younger players: higher potential, lower starting rating

/**
 * Rolls a youth player age using weighted distribution.
 * - 40% chance: age 15-16 (20% each)
 * - 40% chance: age 17
 * - 20% chance: age 18
 */
function rollYouthAge(): number {
  const roll = Math.random();
  if (roll < 0.20) return 15;      // 20% chance
  if (roll < 0.40) return 16;      // 20% chance
  if (roll < 0.80) return 17;      // 40% chance
  return 18;                         // 20% chance
}

/**
 * Age-based rating and potential ranges.
 * Younger players have higher potential but lower starting rating.
 */
const AGE_RATING_CONFIG: Record<number, { potentialRange: [number, number]; ratingRange: [number, number] }> = {
  15: { potentialRange: [65, 90], ratingRange: [30, 45] },
  16: { potentialRange: [65, 90], ratingRange: [30, 45] },
  17: { potentialRange: [60, 85], ratingRange: [38, 52] },
  18: { potentialRange: [55, 80], ratingRange: [45, 58] },
};
 ...
 *
 * BUG-12: Age range changed to 15-18 with weighted distribution.
 * Younger players have higher potential but lower starting rating.
 * Academy level 5 can produce potentials up to 95.
 */
export function generateYouthPlayer(
  ...
  // BUG-12: Use weighted age distribution (15-18 only)
  const age = targetAge ?? rollYouthAge();
  ...
  // BUG-12: Use age-based rating/potential ranges
  const ageConfig = AGE_RATING_CONFIG[age] ?? AGE_RATING_CONFIG[17];

  if (isWonderkid) {
    // Wonderkids: use age config ranges but boost upper bounds
    rating = rng(ageConfig.ratingRange[0], Math.min(ageConfig.ratingRange[1] + 10, 65));
    potential = rng(Math.max(ageConfig.potentialRange[0], 75), Math.min(ageConfig.potentialRange[1] + 5, 95));
    hidden_potential = clamp(potential + rng(2, 8), potential, 99);
  } else {
    const [rMin, rMax] = ageConfig.ratingRange;
    const [pMin, pMax] = ageConfig.potentialRange;

    rating = clamp(rng(rMin, rMax), 25, 65);
    potential = clamp(rng(pMin, pMax), rating + 5, 90);
    hidden_potential = clamp(potential + rng(2, 12), potential, 99);
  }

  // BUG-12: Academy level quality boost
  // Academy level 5 can produce potentials up to 95
  // Each level above 3 adds +3 to potential ceiling
  const academyPotentialBonus = academyLevel >= 5 ? (academyLevel - 4) * 3 : 0;
  const qualityBonus = (clamp(academyLevel, 1, 10) - 1) * 2;
  const finalRating = clamp(rating + qualityBonus, 25, 65);
  const finalPotential = clamp(potential + academyPotentialBonus, finalRating + 5, 95);
  const finalHiddenPotential = clamp(hidden_potential + academyPotentialBonus, finalPotential, 99);

  // Generate stats
  const stats = generateYouthStats(specificPosition, finalRating, rngFn);
  ...
  return {
    ...
    // TASARIM-1: Player Personality (agent message system)
    personality: generateRandomPersonality() as any,
  };
}
...
export function generateScoutReport(player: YouthPlayer): YouthScoutReport {
  ...
  // Age 15: ~40% accurate, Age 18: ~85% accurate
  const accuracyBase = 0.4 + ((player.age - 15) / 3) * 0.45;
...
export function generateYouthIntake(
  ...
  // Age distribution: 50% U17, 35% U18, 15% U19
  const ageDistribution: { range: [number, number]; weight: number }[] = [
    { range: [15, 16], weight: 0.50 },
    { range: [16, 17], weight: 0.35 },
    { range: [17, 18], weight: 0.15 },
  ];
