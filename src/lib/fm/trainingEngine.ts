...
import { TRAINING_PROGRAMS, antrenmanIdDonustur } from './constants';

// ─── Kondisyon-Fitness Çarpanı ──────────────────────────────────────────────
export function getFitnessMultiplier(cond: number): number {
  if (cond >= 90) return 1.0;    // 100% efficiency
  if (cond >= 70) return 0.85;   // 85% efficiency
  if (cond >= 50) return 0.65;   // 65% efficiency
  if (cond >= 30) return 0.40;   // 40% efficiency
  return 0.20;                   // 20% efficiency, increased injury risk
}
...
export function isProgramCompatible(player: Player, programId: string): boolean {
  const program = TRAINING_PROGRAMS.find(p => p.id === antrenmanIdDonustur(programId) || p.id === programId);
...
export const runTrainingSession = (
  squad: Player[],
  state: TrainingState,
  multiplier: number = 1.0,
  options?: { trainingFacilityLevel?: number; coachStars?: number }
) => {
  const coachStars = options?.coachStars || 0;
  ...
  const updatedSquad = squad.map(player => {
    ...
    const program = TRAINING_PROGRAMS.find(p => p.id === antrenmanIdDonustur(assignment.programId) || p.id === assignment.programId);
    ...
    const coachFactor     = 1.0 + coachStars * 0.12; // 5 stars → 1.60x
    const ageFactor       = player.age <= 21 ? 1.15 : player.age >= 30 ? 0.75 : 1.0; // U21 bonus: +15% (eski +50% → dengeleme)
    const fitnessMultiplier = getFitnessMultiplier(player.cond ?? 100);
    ...
    allUniqueStats.forEach(stat => {
      ...
      if (assignment.focusedStat === stat) {
        // Odaklanılan stat: potansiyele doğru açığın %4'ü (eski %10'dan düşürüldü)
        const gap = Math.max(0, potential - currentVal);
        const rawMultiplier = coachFactor * ageFactor * personalityFactor * multiplier * facilityMult * fitnessMultiplier;
        // Çarpan çarpımını maks 3.0x ile sınırla (aşırı büyümeyi önler)
        const cappedMultiplier = Math.min(3.0, rawMultiplier);
        gain = gap * 0.04 * cappedMultiplier;
      } else {
        ...
        const rawMultiplier = intensityFactor * coachFactor * ageFactor * personalityFactor * multiplier * ceilingFactor * facilityMult * fitnessMultiplier;
        // Çarpan çarpımını maks 3.0x ile sınırla (aşırı büyümeyi önler)
        const cappedMultiplier = Math.min(3.0, rawMultiplier);
        gain = (Math.random() * 0.12) * cappedMultiplier;  // 0.15'ten 0.12'ye düşürüldü
      }
    ...
    const condPenalty = cond < 30 ? 0.12 : cond < 40 ? 0.08 : cond < 60 ? 0.03 : 0;
    const injuryRisk = Math.random() < (baseRisk + condPenalty);

    // ── Kondisyon değişimi ────────────────────────────────────────────────
    let condChange = program.condCost ?? -8; // negatif = kayıp, pozitif = kazanç
    // Yoğunluk bazlı kondisyon tüketimi
    const intensity = program.intensity ?? 70;
    if (intensity >= 80) {
      condChange = condChange * 1.25; // Yüksek yoğunluk: daha fazla kondisyon kaybı
    } else if (intensity < 60) {
      condChange = condChange * 0.5;  // Hafif yoğunluk: daha az kondisyon kaybı
    }
    ...
    if (cond < 50) {
      results[player.id].message = 'Düşük kondisyon! Antrenman verimi azalıyor.';
    }
  ...
  try {
    ...
    for (const player of updatedSquad) {
      ...
      if (Object.keys(updates).length > 0) {
        // RPC ile güncelle (RLS uyumlu)
        const { data: rpcResult, error } = await supabase
          .rpc('rpc_save_training_result', {
        ...
        if (error || !rpcResult?.success) {
          errors.push(`Update error for ${player.id}: ${error?.message || rpcResult?.reason || 'RPC failed'}`);
        } else {
          saved++;
        }
      }
