...
import { generateAttributeValue, positionPriorities, specificPositionMentalPriorities, getPositionKey, ATTRIBUTE_KEY_MAP } from './attributeGenerator';
import { calculatePlayerSalary } from './salaryUtils';
import { generateRandomPersonality } from './agentMessageEngine';
...
// Takım kadro şablonu (24 oyuncu — gerçekçi mevki dağılımı)
...
function generatePositionBasedStats(
  ...
  if (!priorities) {
    ...
    return generatePositionBasedStats('MID', specificPosition);
  }

  const stats: Record<string, number> = {};

  // Teknik nitelikler (grup bazlı)
  ...
  // Mental nitelikler — spesifik mevki tablosu öncelikli, yoksa grup bazlı
  const specificMental = specificPosition && specificPositionMentalPriorities[specificPosition];
  if (specificMental) {
  ...
  // Fiziksel nitelikler (grup bazlı)
  ...
  if (posKey === 'GK') {
    ...
    stats.goalkeeping = generateAttributeValue('cok_dusuk'); // 10-40
  }

  // Ek nitelikler: offTheBall (spesifik pozisyona göre)
  const sp = specificPosition || '';
  if (['ST', 'CF'].includes(sp)) {
    stats.offTheBall = stats.offTheBall || generateAttributeValue('yuksek');
  } else if (['LW', 'RW', 'CAM'].includes(sp)) {
    stats.offTheBall = stats.offTheBall || generateAttributeValue('orta_ust');
  } else if (posKey === 'FWD') {
    stats.offTheBall = stats.offTheBall || generateAttributeValue('yuksek');
  } else if (['CM', 'CDM'].includes(sp)) {
    stats.offTheBall = stats.offTheBall || generateAttributeValue('orta');
  } else if (posKey === 'MID') {
    stats.offTheBall = stats.offTheBall || generateAttributeValue('orta');
  } else if (posKey === 'DEF') {
    stats.offTheBall = stats.offTheBall || generateAttributeValue('dusuk_orta');
  } else {
...
export const generatePlayer = (
  ...
  let aStats = generatePositionBasedStats(positionGroup, specificPosition);
  ...
  return {
    ...
    salary: calculatePlayerSalary(baseRating, false, leagueTier || 4),
    nation: 'Turkey',
...
export const generateEliteWonderkid = (): Player => {
  ...
  // Dengeleme: Eski 75-82 OVR çok güçlüydü, 65-73 arası daha uygun
  // Bu hâlâ güçlü bir genç ama doğrudan ilk 11'de oynamamalı
  const rating = 65 + Math.floor(Math.random() * 9); // 65-73 arası
  const player = generateStarterPlayer(pos, rating);
  
  return {
    ...player,
    age: 16 + Math.floor(Math.random() * 2),
    hidden_potential: 85 + Math.floor(Math.random() * 8), // 85-92 arası (eski: 92-99)
    morale: 100,
