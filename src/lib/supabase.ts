...
/**
 * Service-role Supabase client oluşturur (RLS bypass).
 *
 * Cron job'lar ve arka plan işlemleri RLS kısıtlamalarını aşmalıdır.
 * `SUPABASE_SERVICE_ROLE_KEY` ortam değişkeni tanımlıysa onu kullanır;
 * tanımlı değilse anon key'e fallback yapar (geliştirme ortamı).
 *
 * **Önemli:** Bu client yalnızca sunucu tarafında (cron job'lar, API routes)
 * kullanılmalıdır. Asla client component'lere gönderilmemelidir.
 */
