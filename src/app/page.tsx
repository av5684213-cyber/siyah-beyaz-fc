                        ...
                        if (!isFriendly) {
                          ...
                          if (isSupabaseConfigured() && profile.id) {
                            import('@/lib/supabase').then(async ({ getSupabase }) => {
                              ...
                              if (supabase) {
                                try {
                                  const standingsResult = await updateLeagueStandingsAfterClientMatch(
                                    supabase,
                                    profile.id,
                                    results.score?.home ?? 0,
                                    results.score?.away ?? 0
                                  );
                          ...
                          if (isSupabaseConfigured() && profile?.id) {
                            import('@/lib/supabase').then(async ({ getSupabase }) => {
                              ...
                              try {
                                ...
                                await supabase
                                  .from('profiles')
                                  .update({ consecutive_losses: newStreak })
                                  .eq('id', profile.id);
                                ...
                                setProfile((prev: any) => ({ ...prev, consecutive_losses: newStreak }));
                              } catch (e) {
                          ...
                          if (!isFriendly) {
                            import('@/lib/fm/moraleManager').then(({ calculateTeamMoraleUpdates, saveMoraleUpdates }) => {
                              ...
                              const outcome = {
                                result: isWin ? 'W' as const : isLoss ? 'L' as const : 'D' as const,
                                isHome: true,
                                homeScore: homeGoals,
                                awayScore: awayGoals,
                              ...
                              setSquad(prev => {
                                const moraleMap = new Map(moraleUpdates.map(u => [u.playerId, u]));
                                return prev.map(p => {
                                  const update = moraleMap.get(p.id);
                                  if (update) {
                                    return { ...p, morale: update.morale, confidence: update.confidence };
                                  }
                                  return p;
                                });
                              });
