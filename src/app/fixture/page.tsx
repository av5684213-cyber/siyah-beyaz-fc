...
function simulateWeather(matchDate: string): SimulatedWeather {
  // Hava durumu tipi artık merkezi getWeatherForDate() ile deterministik
  const type = getWeatherForDate(matchDate);

  // Sıcaklık hesaplama: tarihe göre mevsimsel
  ...
  // Tip → etiket & ikon eşlemesi
  switch (type) {
    case 'sunny':
      return { type, temperature, label: 'Güneşli', icon: <CloudSun size={16} className="text-amber-400" /> };
    case 'rainy':
      return { type, temperature, label: 'Yağmurlu', icon: <CloudRain size={16} className="text-sky-400" /> };
    case 'windy':
      return { type, temperature, label: 'Rüzgarlı', icon: <Wind size={16} className="text-white/50" /> };
    case 'snowy':
      return { type, temperature: Math.min(temperature, 2), label: 'Karlı', icon: <CloudRain size={16} className="text-blue-300" /> };
  }
}
