...
export default function PlayerDetailPage() {
  ...
  // ──── Personality object (new format) ────
  const personalityObj = (player.personality && typeof player.personality === 'object' && ('ambition' in (player.personality as Record<string, unknown>) || 'pressure_handling' in (player.personality as Record<string, unknown>)))
    ? player.personality as { ambition: number; professionalism: number; temperament: number; loyalty: number; pressure_handling: number }
    : undefined;
    ...
    <div className="min-h-screen bg-black text-white p-4 md:p-6">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          {/* Takım rozeti - en üstte */}
          <div className="mb-4">
            {player.team_name || player.club ? (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-sm bg-amber-500/10 border border-amber-500/20 text-[10px] font-black uppercase tracking-[0.15em] text-amber-300">
                <Shield size={11} className="text-amber-400" />
                {toTitleCase(player.team_name || player.club || '')}
              </span>
            ) : (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-sm bg-white/[0.03] border border-white/10 text-[10px] font-black uppercase tracking-[0.15em] text-white/30">
              ...
              <div className="flex flex-wrap gap-2 mt-2 text-[10px] font-bold text-white/40">
                ...
                {player.preferred_foot && (
                  <span className="px-2 py-1 rounded-lg bg-white/5 border border-white/10">
                    {player.preferred_foot === 'Left' || player.preferred_foot === 'Sol' ? 'Sol' : player.preferred_foot === 'Right' || player.preferred_foot === 'Sağ' ? 'Sağ' : 'Her İki'} Ayak
                  </span>
                )}
              </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
          ...
          {traits.length === 0 && negTraits.length === 0 && personalityTraits.length === 0 && !personalityObj ? (
            ...
            <div className="space-y-4">
              ...
              {/* Personality Attributes (new format: ambition, professionalism, etc.) */}
              {personalityObj && (
                <div>
                  <div className="text-[8px] font-black uppercase tracking-[0.2em] text-white/25 mb-2">Kişilik Değerleri</div>
                  <div className="grid grid-cols-5 gap-2">
                    {[
                      { key: 'ambition', label: 'Hırslılık', val: personalityObj.ambition },
                      { key: 'professionalism', label: 'Profesyonellik', val: personalityObj.professionalism },
                      { key: 'temperament', label: 'Mizaç', val: personalityObj.temperament },
                      { key: 'loyalty', label: 'Sadakat', val: personalityObj.loyalty },
                      { key: 'pressure_handling', label: 'Baskı Dayanıklılığı', val: personalityObj.pressure_handling },
                    ].map(attr => (
                      <div key={attr.key} className="text-center bg-white/[0.03] rounded-xl p-2 border border-white/5">
                        <p className="text-[7px] font-black text-white/30 uppercase tracking-widest">{attr.label}</p>
                        <p className={`text-sm font-mono font-bold ${
                          attr.val >= 15 ? 'text-emerald-400' : attr.val >= 10 ? 'text-amber-400' : 'text-red-400'
                        }`}>{attr.val}</p>
                      </div>
                    ))}
                  </div>
