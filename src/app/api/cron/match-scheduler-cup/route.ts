/**
 * Cron Job: Kupa Maç Başlatıcı
 *
 * Cumartesi ve Pazar günleri 18:00 İstanbul saatinde
 * kupa fikstürlerini canlıya alır.
 * Lig maç scheduler'dan ayrı çalışır.
 *
 * GET /api/cron/match-scheduler-cup
...
// MatchScheduleManager: Kupa takvim kuralları
...
/** Kupa maç scheduler için kilit adı ve TTL */
const LOCK_NAME = 'match-scheduler-cup';
...
export async function GET(request: NextRequest) {
  ...
  try {
    console.log('[cron/match-scheduler-cup] Kupa maç başlatıcı başlıyor...');

    const istNow = getIstanbulDateTime(new Date());
    const today = istNow.dateStr;
    const dayOfWeek = istNow.dayOfWeek;
    const dayNames = ['Pazar','Pazartesi','Salı','Çarşamba','Perşembe','Cuma','Cumartesi'];
    console.log(`[cron/match-scheduler-cup] İstanbul: ${today} ${istNow.timeStr} (${dayNames[dayOfWeek]})`);

    // ── Takvim kuralı: Bugün kupa maçı olmalı mı? ──
    if (!shouldPlayCup(dayOfWeek)) {
      console.log(`[cron/match-scheduler-cup] Bugün kupa maçı yok (${dayNames[dayOfWeek]}). Atlanıyor.`);
      return NextResponse.json({
        success: true,
        message: `Bugün kupa maçı yok (${dayNames[dayOfWeek]})`,
        started: 0,
        timestamp: new Date().toISOString(),
      });
    }

    // Kupa fikstürlerini getir
    const { data: pendingFixtures, error: fixturesError } = await supabase
      ...
      .eq('status', 'scheduled')
      .eq('match_date', today)
      .eq('competition_type', 'cup') // Sadece kupa maçları
      .limit(50);

    if (fixturesError) {
      console.error('[cron/match-scheduler-cup] Fikstür sorgu hatası:', fixturesError);
      return createErrorResponse(fixturesError, { route: '/api/cron/match-scheduler-cup', method: 'GET' });
    }

    if (!pendingFixtures || pendingFixtures.length === 0) {
      return NextResponse.json({
        success: true,
        message: 'Bugün kupa fikstürü bulunamadı',
        started: 0,
        timestamp: new Date().toISOString(),
      });
    }

    // Saat kontrolü: kupa maçları 18:00 İstanbul
    ...
    if (readyFixtures.length === 0) {
      return NextResponse.json({
        success: true,
        message: 'Kupa maç saati henüz gelmedi',
        started: 0,
        timestamp: new Date().toISOString(),
      });
    }

    console.log(`[cron/match-scheduler-cup] ${readyFixtures.length} kupa maçı başlatılıyor`);

    // Her kupa fikstürü için maç oturumu oluştur
    for (const fixture of readyFixtures) {
      try {
        ...
        if (existingSession) {
          console.log(`[match-scheduler-cup] Fikstür ${fixture.id} zaten canlı (${existingSession.status})`);
          continue;
        }
        ...
        try {
          ...
          console.warn('[match-scheduler-cup] Hakem atama hatası:', refErr);
        }
        ...
        await supabase.from('match_events').insert({
          ...
          detail: `Kupa maçı başlıyor! 🏆 Hava: ${weatherTr[weather] || 'Güneşli'}.`,
          is_revealed: true,
        });
        ...
        try {
          ...
          console.warn('[match-scheduler-cup] live_matches ekleme hatası:', liveErr);
        }
        ...
        // Push bildirimleri: Kupa maçı başladı!
        const pushTitle = '🏆 Kupa Maçı Başladı!';
        const pushBody = `${homeTeamData.name} vs ${awayTeamData.name} — Kupa maçı canlı!`;
        ...
        console.log(`[match-scheduler-cup] Kupa oturumu: ${homeTeamData.name} vs ${awayTeamData.name}`);

      } catch (err) {
        errors.push(`Fikstür ${fixture.id}: ${err}`);
        console.error(`[match-scheduler-cup] Fikstür hatası ${fixture.id}:`, err);
      }
    }
    ...
    return createErrorResponse(err, { route: '/api/cron/match-scheduler-cup', method: 'GET' });
  } finally {
    // ── Cron Lock bırak ──
    if (lock) {
