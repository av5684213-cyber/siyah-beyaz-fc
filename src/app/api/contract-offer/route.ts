...
// ── Player Demands Generator (same as frontend) ──────────────────────
function generatePlayerDemands(rating: number) {
  if (rating >= 90) {
    return { minWeeklySalary: 600000, maxWeeklySalary: 1000000, minSigningFee: 30, maxSigningFee: 50 };
  } else if (rating >= 80) {
    return { minWeeklySalary: 300000, maxWeeklySalary: 600000, minSigningFee: 15, maxSigningFee: 30 };
  } else if (rating >= 70) {
    return { minWeeklySalary: 150000, maxWeeklySalary: 300000, minSigningFee: 8, maxSigningFee: 15 };
  } else {
    return { minWeeklySalary: 50000, maxWeeklySalary: 150000, minSigningFee: 3, maxSigningFee: 8 };
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ accepted: false, reason: 'Beklenmeyen bir hata olustu.' }, { status: 500 });
  }
}
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    console.error('Auction contract error:', err);
    return NextResponse.json({ accepted: false, reason: 'Beklenmeyen bir hata olustu.' }, { status: 500 });
  }
}
