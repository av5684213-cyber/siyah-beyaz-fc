...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    for (const league of (leagues || [])) {
      ...
      if (teamCount === 0) {
        ...
        const teamsToInsert = names.map((name, idx) => ({
          ...
          is_npc: true,
          is_bot: true, // Kullanıcı kaydında bot devralma için gerekli
        }));
        ...
        if (!standingsCount || standingsCount === 0) {
          ...
          if (seasonTeams && seasonTeams.length > 0) {
            const standingsRows = seasonTeams.map((t: { id: string }) => ({
              ...
              team_id: t.id,
              league_id: league.id, // BUG #2 FIX: league_id eksikti, season-end bunu kullanıyor
      ...
      const now = new Date();
    const year = `${now.getFullYear()}/${(now.getFullYear() + 1).toString().slice(2)}`;
    ...
    return createErrorResponse(error, { route: '/api/league/maintenance', method: 'GET' });
  }
}
