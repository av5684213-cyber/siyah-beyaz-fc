...
export async function POST(request: NextRequest) {
  try {
    ...
    // -- Get user profile (credits + money) --
    const { data: profile, error: profileError } = await supabase
      .from('profiles')
      .select('credits, money, league_name')
      .eq('id', userId)
      .maybeSingle();
    ...
    if (insertError) {
      ...
      // Refund on failure
      await supabase.from('profiles').update({ credits: profile.credits, money: profile.money }).eq('id', userId);
    ...
    console.error('[POST /api/staff/hire] Exception:', err);
