...
export async function POST(request: NextRequest) {
  try {
    ...
    const { profileId: bodyProfileId, facilityType, action } = body;
    ...
    if (action === 'start') {
      ...
      await supabase.rpc('rpc_update_profile', { p_profile_id: profileId, p_updates: { credits: newCredits } });
    ...
    return createErrorResponse(err, { route: '/api/facilities/upgrade', method: 'POST' });
  }
}
