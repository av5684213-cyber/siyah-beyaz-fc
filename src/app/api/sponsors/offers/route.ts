...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return createErrorResponse(err, { route: '/api/sponsors/offers', method: 'GET' });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { sponsorId, profileId: bodyProfileId, signedWeek, weeklyIncome, durationWeeks } = body;
    ...
    if (!valid) {
      return NextResponse.json({ error: profileError || 'Geçersiz profil ID' }, { status: profileStatus || 400 });
    }
    ...
    if (error) {
      ...
      return createErrorResponse(error, { route: '/api/sponsors/offers', method: 'POST' });
    }

    // Update profile.sponsors JSONB with the new sponsor entry
    if (data && !error) {
      try {
        ...
        const currentSponsors = Array.isArray(profileData?.sponsors) ? profileData.sponsors : [];
        const newSponsorEntry = {
          id: sponsorId,
          name: sponsor.name,
          logo: sponsor.logo,
          weeklyPayment: weekly,  // MUST match weekly-income cron's `s.weeklyPayment` key
        ...
        await supabase
          .from('profiles')
          .update({ sponsors: [...currentSponsors, newSponsorEntry] })
          .eq('id', profileId);
      } catch (profileUpdateErr) {
        console.warn('[POST /api/sponsors/offers] Profile sponsors JSONB update failed:', profileUpdateErr);
      }
    }
    ...
    return createErrorResponse(err, { route: '/api/sponsors/offers', method: 'POST' });
  }
}
