...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // ── Bakiye kontrolü ──
    if ((renterProfile.credits || 0) < RENTAL_COMMISSION_KR) {
      console.warn('[POST /api/rental/offer] Insufficient credits. renterTeamId:', renterTeamId, 'required:', RENTAL_COMMISSION_KR, 'available:', renterProfile.credits || 0);
      return NextResponse.json({
        error: `Yetersiz kredi. Kiralama komisyonu: ${RENTAL_COMMISSION_KR} KR. Mevcut: ${renterProfile.credits || 0} KR`,
        userMessage: `Yetersiz kredi bakiye. Kiralama komisyonu ${RENTAL_COMMISSION_KR} KR gerektiriyor ancak mevcut bakiyeniz ${renterProfile.credits || 0} KR. Lütfen kredi yükleyin.`,
        debug: { required: RENTAL_COMMISSION_KR, available: renterProfile.credits || 0 },
      }, { status: 400 });
    }
    ...
    const { error: commissionDeductError } = await supabase
      .rpc('rpc_update_profile', { p_profile_id: renterTeamId, p_updates: { credits: (renterProfile.credits || 0) - RENTAL_COMMISSION_KR } });

    if (commissionDeductError) {
      console.error('[POST /api/rental/offer] Failed to deduct commission from renter. renterTeamId:', renterTeamId, 'commission:', RENTAL_COMMISSION_KR, 'error:', commissionDeductError.message);
      return NextResponse.json({
        ...
        debug: { renterTeamId, commission: RENTAL_COMMISSION_KR, error: commissionDeductError.message },
      }, { status: 500 });
    }

    // ── Euro ücreti alıcıdan düş, satıcıya ekle ──
    const { error: euroDeductError } = await supabase
      .rpc('rpc_update_profile', { p_profile_id: renterTeamId, p_updates: { money: (renterProfile.money || 0) - totalCostEuro } });
    ...
    if (euroDeductError) {
      ...
      // Komisyonu geri öde
      await supabase.rpc('rpc_update_profile', { p_profile_id: renterTeamId, p_updates: { credits: renterProfile.credits || 0 } });
      return NextResponse.json({
    ...
    if (ownerProfile) {
      const { error: ownerPaymentError } = await supabase
        .rpc('rpc_update_profile', { p_profile_id: listing.owner_team_id, p_updates: { money: (ownerProfile.money || 0) + totalCostEuro } });
    ...
    const { data: agreement, error: agreementError } = await supabase
      ...
      .insert({
        ...
        commission: RENTAL_COMMISSION_KR,
    ...
    if (agreementError) {
      ...
      try {
        await supabase.rpc('rpc_update_profile', { p_profile_id: renterTeamId, p_updates: { credits: renterProfile.credits || 0, money: renterProfile.money || 0 } });
        if (ownerProfile) {
          await supabase.rpc('rpc_update_profile', { p_profile_id: listing.owner_team_id, p_updates: { money: ownerProfile.money || 0 } });
        }
    ...
    const { error: playerUpdateError } = await supabase
      ...
      .eq('id', playerId)
      .eq('profile_id', listing.owner_team_id);
