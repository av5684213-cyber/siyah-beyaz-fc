...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Bu olmadan herkes başka bir kullanıcının oyuncusunu kiralık listesine ekleyebilir
    ...
    if (expectedOwner && player.profile_id && player.profile_id !== expectedOwner) {
      console.warn('[POST /api/rental/create-listing] Ownership mismatch. playerId:', playerId, 'expectedOwner:', expectedOwner, 'actualOwner:', player.profile_id);
      return NextResponse.json({ error: 'Bu oyuncu sizin kadronuzda değil' }, { status: 403 });
    }
    ...
    return createErrorResponse(err, { route: '/api/rental/create-listing', method: 'POST' });
  }
}
