...
export default function AwardsPage() {
  ...
  // Kullanıcı ID'sini al (S3-7 FIX: auth çözülene kadar bekler)
  useEffect(() => {
  ...
  useEffect(() => {
    // S3-7 FIX: auth çözülmeden sorgu yapma — userId null iken TÜM ödüller geliyordu
    ...
    const loadAwards = async () => {
      try {
        ...
        // S3-7 FIX: Kullanıcı giriş yapmamışsa ödül gösterme
        if (!userId) {
          setAwards([]);
          setLoading(false);
          return;
