...
-- ═══════════════════════════════════════════════════════════════════════════
-- BÖLÜM 12: SIM-3 FIX — cron_locks RLS politikası ekle
-- ═══════════════════════════════════════════════════════════════════════════
-- Service role key ile cron joblar kilitleme yapabilmeli
-- Ayrıca anon key de okuyabilmeli (durum kontrolü için)

DO $$ BEGIN
  CREATE POLICY "cron_locks_select_all" ON cron_locks FOR SELECT USING (true);
  CREATE POLICY "cron_locks_insert_service" ON cron_locks FOR INSERT WITH CHECK (true);
  CREATE POLICY "cron_locks_update_service" ON cron_locks FOR UPDATE USING (true);
  CREATE POLICY "cron_locks_delete_service" ON cron_locks FOR DELETE USING (true);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

-- ═══════════════════════════════════════════════════════════════════════════
-- BÖLÜM 13: SIM-7 FIX — 'finished' durumundaki fikstürleri 'completed'a çevir
-- ═══════════════════════════════════════════════════════════════════════════

UPDATE fixtures SET status = 'completed' WHERE status = 'finished';

-- ═══════════════════════════════════════════════════════════════════════════
-- BÖLÜM 14: SIM-9 FIX — Toplu sezon sıfırlama RPC fonksiyonu
-- ═══════════════════════════════════════════════════════════════════════════
-- Sezon sonunda tüm oyuncuların istatistiklerini tek sorguda sıfırlar
-- ve yaşlarını 1 artırır. Bu, 2000+ ayrı UPDATE sorgusu yerine tek RPC çağrısıdır.

CREATE OR REPLACE FUNCTION batch_season_reset()
RETURNS void AS $$
BEGIN
  -- Oyuncu sezon istatistiklerini sıfırla
  UPDATE players SET
    goals = 0,
    assists = 0,
    yellow_cards = 0,
    ...
    clean_sheets = 0,
    age = age + 1,
    rating_start_of_season = rating
  WHERE age IS NOT NULL;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- ═══════════════════════════════════════════════════════════════════════════
-- BÖLÜM 15: SIM-4 FIX — Stuck sezonları tamamla
-- ═══════════════════════════════════════════════════════════════════════════
-- Tüm fikstürleri tamamlanmış ama is_finished=false olan sezonları bul ve tamamla

UPDATE seasons SET is_finished = true, status = 'completed'
WHERE is_finished = false
  AND id IN (
    ...
    WHERE NOT EXISTS (
      ...
      AND f.status IN ('scheduled', 'live', 'in_progress')
    )
    AND EXISTS (
      SELECT 1 FROM fixtures f
      WHERE f.season_id = s.id
...
-- 1-11. Önceki migration'ların tüm değişiklikleri
-- 12. cron_locks tablosuna RLS politikası eklendi
-- 13. 'finished' durumundaki fikstürler 'completed'a çevrildi
-- 14. batch_season_reset() RPC fonksiyonu oluşturuldu
-- 15. Stuck sezonlar tamamlandı (is_finished=true)
-- ═══════════════════════════════════════════════════════════════════════════
